...
bool Pathfinder::FindPath(float sx, float sy, float ex, float ey)
{
    vec2 TileSize = m_pTilemap->GetTileSize();
    return FindPath((int)(sx / TileSize.x), (int)(sy / TileSize.y), (int)(ex / TileSize.x), (int)(ey / TileSize.y));
}
...
int Pathfinder::GetPath(int* path, int maxdistance, float ex, float ey)
{
    vec2 TileSize = m_pTilemap->GetTileSize();
    return GetPath(path, maxdistance, (int)(ex / TileSize.x), (int)(ey / TileSize.y));
}
...
int Pathfinder::CheckIfNodeIsClearAndReturnNodeIndex(int tx, int ty)
{
    ...
    if( m_pTilemap->IsTileWalkable( tx, ty ) == false )
        return -1;
